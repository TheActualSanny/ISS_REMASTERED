...
class JSON_Manager:
    def __init__(self):
        ...
        self._user_params = {'User-Agent' : parser['USER_DATA']['UserAgent']} 
    
    def fetch_json(self) -> dict:
        '''Method which send a request to the API and gets the ISS data.'''
        raw_json = requests.get(parser['CRUCIAL']['URL'], params = self._user_params)
    ...
    def update_json(self) -> None:
        ...
        with open(f'{parser['CRUCIAL']['LAKE_NAME']}.json', 'r') as lake:
            existing_data = json.load(lake)
        
        existing_data['iss'].append(new_data)
        with open(f'{parser['CRUCIAL']['LAKE_NAME']}.json', 'w') as lake:
